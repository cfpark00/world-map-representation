...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/result1-1.png}
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/result1-2.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/result2-1.png}
...
Fig.~\ref{fig:app_reprs} shows PCA projections of city representations for single-task models across three random seeds.
...
Fig.~\ref{fig:app_training} shows training dynamics for all seven single-task models.

\begin{figure*}[h!]
    \centering
    \includegraphics[width=\textwidth]{figures/app_training_curves.png}
    \caption{\textbf{Training dynamics for all single-task models.} (a) \texttt{distance}, (b) \texttt{trianglearea}, (c) \texttt{angle}, (d) \texttt{compass}, (e) \texttt{inside}, (f) \texttt{perimeter}, (g) \texttt{crossing}.}
    \label{fig:app_training}
\end{figure*}

\paragraph{Representation Dynamics.}
Fig.~\ref{fig:app_repr_dynamics} visualizes how internal representations evolve during training via PCA projections at six checkpoints.

\begin{figure*}[h!]
    \centering
    \includegraphics[width=\textwidth]{figures/app_repr_dynamics.png}
    \caption{\textbf{Representation dynamics during training.} Rows: \texttt{distance} (top), \texttt{angle} (middle), \texttt{compass} (bottom). Columns show PCA projections at gradient steps 8204, 24612, 49224, 123060, 188692, and 328146 (left to right).}
...
Fig.~\ref{fig:app_cka_pt1} shows CKA matrices for single-task models at layers 3, 4, 5 and 6.

\begin{figure*}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/app_cka_pt1.png}
    \caption{\textbf{CKA matrices for single-task models across layers.} Each cell shows mean $\pm$ SEM across 3 seeds. D=distance, T=triangle area, A=angle, Co=compass, I=inside, P=perimeter, Cr=crossing.}
    \label{fig:app_cka_pt1}
\end{figure*}

\paragraph{Two-Task CKA.}
Fig.~\ref{fig:app_cka_pt2} shows the CKA matrix for two-task models at layer 5.

\begin{figure*}[h!]
    \centering
    \includegraphics[width=0.4\textwidth]{figures/app_cka_pt2.png}
    \caption{\textbf{CKA matrix for two-task models at layer 5.} Mean $\pm$ SEM across 3 seeds. All pairs show high alignment ($>$0.84).}
    \label{fig:app_cka_pt2}
\end{figure*}

\paragraph{CKA vs.\ Task Count (Per-Seed).}
Fig.~\ref{fig:app_cka_3seed} shows the same CKA vs.\ task count analysis broken down by individual seeds.

\begin{figure*}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/app_cka_3seed.png}
    \caption{\textbf{CKA vs.\ task count for individual seeds.} Each panel shows a different seed.}
    \label{fig:app_cka_3seed}
\end{figure*}

\paragraph{Aggregated CKA Trends.}
Fig.~\ref{fig:app_cka_additional} shows CKA vs.\ task count using all two-task and three-task models.

\begin{figure*}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/app_cka_additional.png}
    \caption{\textbf{Aggregated CKA analysis.} (a) CKA vs.\ task count for single seed. (b) Within-task CKA increases with task count.}
    \label{fig:app_cka_additional}
\end{figure*}

\paragraph{CKA vs.\ Generalization (Annotated).}
Fig.~\ref{fig:app_cka_vs_ni_annotated} is an annotated version of Fig.~\ref{fig:result2-1}(b).

\begin{figure*}[h!]
    \centering
    \includegraphics[width=0.4\textwidth]{figures/app_cka_vs_ni_annotated.png}
    \caption{\textbf{Annotated version of Fig.~\ref{fig:result2-1}(b).} Each point is labeled with its (train$\rightarrow$eval) task pair.}
...
\begin{figure*}[h!]
    ...
    \caption{\textbf{Single-task fine-tuning results for individual seeds.} Per-seed version of Fig.~\ref{fig:result2-1}(a).}
...
\begin{figure*}[h!]
    ...
    \caption{\textbf{Deviation from best-teacher expectation for all 21 two-task combinations.} All 4 seeds shown.}
...
Fig.~\ref{fig:app_atlantis_in_pt} shows the resulting representations when \texttt{Atlantis} cities are included from the start of pretraining.

\begin{figure*}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/app_atlantis_in_pt.png}
    \caption{\textbf{Representations when \texttt{Atlantis} is included during pretraining.} (a) PCA projection showing \texttt{Atlantis} cities integrated with world cities. (b) Linear probe reconstruction confirming geographic accuracy.}
    \label{fig:app_atlantis_in_pt}
\end{figure*}

\paragraph{Wider Model.}
Fig.~\ref{fig:app_wide_stat} shows fine-tuning results for a wider model with 2$\times$ the hidden dimension.

\begin{figure*}[h!]
    \centering
    \includegraphics[width=\textwidth]{figures/app_wide_stat.png}
    \caption{\textbf{Fine-tuning results for wider model (2$\times$ hidden dimension).} (a) Single-task fine-tuning normalized improvement. (b) Two-task fine-tuning normalized improvement. (c) Deviation from best-teacher expectation.}
...
\textbf{Interpretability \& Internal Representations.}
Understanding internal representations has roots in neuroscience, informing early neural network development. Beyond the world model discoveries cited in Sec.~\ref{sec:related}, similar representations emerge during in-context learning. Researchers have also uncovered that models represent meaningful properties of data, concepts, features and abstractions, in interpretable ways.

\textbf{Fine-tuning.}
Beyond the works cited in Sec.~\ref{sec:related}, fine-tuning has been studied extensively across diverse directions: parameter efficiency, zeroth-order optimization, weight composition and representation adaptation.

\textbf{Dynamics of Representations.}
Recent work has begun studying how representations evolve during in-context learning or fine-tuning. How internal representations adapt at inference time is an active area of research.

\textbf{Loss Plateaus.}
Our \texttt{crossing} task fails to learn in single-task training despite escaping an initial plateau. Such plateaus are notoriously difficult for transformers. Multi-task training has been shown to shorten loss plateaus, similar to why our \texttt{crossing} task trains successfully when joined with any other task.
