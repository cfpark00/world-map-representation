...
def main():
    ...
    parser.add_argument('--debug', action='store_true', help='Debug mode: limit to 50 cities')
    ...
    source_csv = config.get('source_csv', '/data/geonames-all-cities-with-a-population-1000.csv')
    ...
    # Debug mode: limit to 50 cities
    if args.debug:
        print(f"\n[DEBUG MODE] Limiting to 50 cities (was {len(filtered_df)})")
        filtered_df = filtered_df.head(50).reset_index(drop=True)
    ...
    with open('data/geographic_mappings/country_to_region.json', 'r') as f:
