...
for i in range(n):
    for j in range(i, n):  # Upper triangle + diagonal
        ...
        result_dir = base_dir / f'{exp1_name}_vs_{exp2_name}' / 'layer6'
        summary_file = result_dir / 'summary.json'

        # If not found, try the reverse ordering
        if not summary_file.exists():
            result_dir = base_dir / f'{exp2_name}_vs_{exp1_name}' / 'layer6'
            summary_file = result_dir / 'summary.json'
...
# Title
title = f'21×21 CKA Matrix (Layer 5)\n'
...
# Save 7x7 with SEM
output_path_7x7_sem = output_dir / 'cka_matrix_7x7_averaged_layer6_with_sem.png'
plt.savefig(output_path_7x7_sem, dpi=300, bbox_inches='tight')
print(f"Saved 7x7 plot with SEM to: {output_path_7x7_sem}")
...
# Save bar plot
output_path_bar = output_dir / 'cka_barplot_intra_vs_inter_layer6.png'
