#!/usr/bin/env python3
"""
Create FTWB2 vs FTWB1 prediction plots for revision/exp3.
Generates 2 plots (wide + narrow), each showing:
- Difference from predicted (actual - predicted) using RdBu colormap

The prediction is based on maximum performance from single-task FTWB1 models
from the SAME model type (e.g., wide FTWB2 predicted from wide FTWB1).
"""
...
def create_actual_matrix(ftwb2_perf, baselines, baselines_atlantis):
    """Create normalized actual performance matrix (7 tasks × 6 experiments)."""
    n_tasks = len(TASKS)
...
def plot_ftwb2_vs_ftwb1(model_type):
    ...
    # Transpose matrices: rows=experiments, cols=tasks
    actual_transposed = actual_matrix.T
    diff_transposed = diff_matrix.T

    # Difference from predicted (Actual - Predicted) - NO colorbar
    sns.heatmap(diff_transposed, annot=True, fmt=".2f", cmap='RdBu',
    ...
    # Save figure
    filename = f"{model_type}_ftwb2_vs_ftwb1.png"
    ...
    for exp_idx, exp_num in enumerate(FTWB2_EXPS):
        for task in TRAINING_DATA_2TASK[exp_num]:
            task_idx = TASKS.index(task)
            trained_actual.append(actual_transposed[exp_idx, task_idx])
            trained_diff.append(diff_transposed[exp_idx, task_idx])
    ...
    for exp_idx, exp_num in enumerate(FTWB2_EXPS):
        ...
        for task_idx, task in enumerate(TASKS):
            if task not in trained_tasks:
                transfer_actual.append(actual_transposed[exp_idx, task_idx])
                transfer_diff.append(diff_transposed[exp_idx, task_idx])
...
def main():
    print("="*60)
    print("Creating FTWB2 vs FTWB1 Plots for Revision Exp3")
    print("="*60)
    ...
    for model_type in MODEL_TYPES:
        ...
        if not (base_exp / "evals").exists():
            ...
            continue
        plot_ftwb2_vs_ftwb1(model_type)
    ...
    print("\nGenerated files:")
    print("  - wide_ftwb2_vs_ftwb1.png")
    print("  - narrow_ftwb2_vs_ftwb1.png")
