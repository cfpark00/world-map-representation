"""
Analyze CKA between two experiments after PCA dimensionality reduction.

Each experiment gets its own PCA transformation fitted on non-Atlantis cities.
Then CKA is computed on the PCA-reduced representations (default: 3 components).
"""
import sys
...
def apply_pca_to_representations(repr_dict, meta_dict, city_filter, n_components=5):
    """
...
def plot_cka_timeline(df, output_path, summary):
    ...
    ax.set_title(f"CKA Timeline (First {summary['n_pca_components']} PCs): {summary['exp1']} vs {summary['exp2']}\nLayer {summary['layer']}", fontsize=14)
...
def main(config_path, overwrite=False, debug=False):
    """Analyze CKA between two experiments using PCA-reduced representations."""
    ...
    print(f"\nComputing CKA on PCA-reduced representations...")
    results = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Analyze CKA between two experiments using PCA-reduced representations')
