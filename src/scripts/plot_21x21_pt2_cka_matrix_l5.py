#!/usr/bin/env python3
"""
Generate 21×21 CKA matrix for PT2 layer 5.
...
# PT2 task definitions for overlap detection
PT2_TASKS = {
...
def get_overlap_type(var1, var2):
    """
    Returns overlap type between two PT2 variants:
    - 'none': no shared tasks
    - 'partial': exactly 1 shared task
    - 'full': same variant (2 shared tasks)
    """
    if var1 == var2:
        return 'full'
    tasks1 = PT2_TASKS[var1]
    tasks2 = PT2_TASKS[var2]
    shared = len(tasks1 & tasks2)
    if shared == 0:
        return 'none'
    elif shared == 1:
        return 'partial'
...
def load_cka_matrix(base_path):
    """Load CKA values from all PT2 pairs."""
    ...
    for i, exp1 in enumerate(experiments):
        for j, exp2 in enumerate(experiments):
            ...
            # Try both orderings
            pair_name = f'pt2-{exp1.split("-")[1]}_vs_pt2-{exp2.split("-")[1]}'
...
def plot_21x21_matrix(matrix, experiments, output_dir):
    ...
    # Remove axis labels and ticks completely
    ax.set_xticks([])
    ax.set_yticks([])

    ax.set_title('PT2 CKA Matrix (21×21, Layer 5)', fontsize=16, fontweight='bold', pad=20)
    ...
    # Add thicker grid lines to separate models (every 3 experiments = 1 model)
    ...
    output_file = output_dir / 'pt2_cka_21x21_l5.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Saved: {output_file}")
    plt.close()

    # Save matrix as CSV
    csv_file = output_dir / 'pt2_cka_21x21_l5.csv'
...
def plot_7x7_averaged(matrix, experiments, output_dir):
    ...
    # Add annotations and red triangles for partial overlap
    for i in range(n_var):
        for j in range(n_var):
            text_color = 'white' if avg_matrix[i, j] < 0.6 else 'black'
            ax.text(j, i, f'{avg_matrix[i, j]:.2f}',
                   ha='center', va='center', color=text_color, fontsize=24)

            # Add tiny red triangle in corner for partial overlap cells
            if get_overlap_type(i + 1, j + 1) == 'partial':
                # Triangle in top-right corner of cell
                triangle = Polygon(
                    [(j + 0.5, i - 0.5), (j + 0.5, i - 0.25), (j + 0.25, i - 0.5)],
                    closed=True, facecolor='red', edgecolor='none', zorder=10
                )
    ...
    # Add grid with minor ticks
    ax.set_xticks(np.arange(n_var) - 0.5, minor=True)
    ax.set_yticks(np.arange(n_var) - 0.5, minor=True)
    ax.grid(which='minor', color='gray', linestyle='-', linewidth=0.5)

    plt.tight_layout()

    output_file = output_dir / 'pt2_cka_7x7_averaged_l5.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Saved: {output_file}")
    plt.close()
    ...
    # Add annotations with SEM and red triangles for partial overlap
    for i in range(n_var):
        for j in range(n_var):
            text_color = 'white' if avg_matrix[i, j] < 0.6 else 'black'
            text = f'{avg_matrix[i, j]:.2f}\n±{sem_matrix[i, j]:.3f}'
            ax.text(j, i, text,
    ...
    output_file = output_dir / 'pt2_cka_7x7_averaged_with_sem_l5.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Saved: {output_file}")
    plt.close()
...
def plot_intra_vs_inter_bar(matrix, experiments, output_dir):
    ...
    ax.set_title(f'PT2 Layer 5: Intra-task vs Inter-task CKA\n(p={p_value:.4f})',
    ...
    output_file = output_dir / 'pt2_cka_intra_vs_inter_l5.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Saved: {output_file}")
    plt.close()
