#!/usr/bin/env python3
"""
Triangle area dataset creation script.
Generates triples of cities and calculates the area of the triangle they form.
"""
import pandas as pd
import numpy as np
import sys
...
def generate_all_triples(df, n_triples):
    ...
    # Filter out degenerate triangles (where any two cities are the same)
...
def generate_must_include_triples(df, must_include_groups, n_triples):
    ...
    for _ in range(n_triples):
        ...
        # Randomly decide how many (1, 2, or 3)
        n_must = np.random.choice([1, 2, 3], p=[0.6, 0.3, 0.1])

        if n_must == 1:
            # One from must_include, two from all
            i = np.random.choice(must_include_indices)
            j, k = np.random.choice(all_indices, size=2, replace=False)
        elif n_must == 2:
            # Two from must_include, one from all
            i, j = np.random.choice(must_include_indices, size=2, replace=False)
            k = np.random.choice(all_indices)
        else:
...
def create_dataset_dict(indices_i, indices_j, indices_k, df, tokenizer, config=None):
    ...
    areas = calculate_triangle_area(x1, y1, x2, y2, x3, y3)
    areas = np.round(areas).astype(int)
    ...
    for c1, c2, c3, area in tqdm(zip(city1_ids, city2_ids, city3_ids, areas),
                                  total=len(city1_ids),
        ...
        if use_padding:
            ...
            area_str = f"triarea(c_{c1_str},c_{c2_str},c_{c3_str})={area}"
        else:
            area_str = f"triarea(c_{c1},c_{c2},c_{c3})={area}"

        # Add spaces between each character for tokenizer
        spaced_str = ' '.join(area_str)
        text = f"<bos> {spaced_str} <eos>"

        text_list.append(text)
        task_type_list.append("trianglearea")
...
def main():
    parser = argparse.ArgumentParser(description='Create triangle area dataset')
